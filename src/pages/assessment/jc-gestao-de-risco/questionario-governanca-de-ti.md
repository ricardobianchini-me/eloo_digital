...
Cliente: JC Gestão de Risco | Módulo: Governança de TI
Setor: Gestão de Risco | Porte: Médio | Interlocutor: (a definir no kickoff)
...
| GOV-06 | Incidentes recorrentes ou de alta criticidade geram comunicação formal às áreas de negócio impactadas (ex.: operação da empresa e sistemas de gestão de risco)? | Exemplo de comunicado ou registro de notificação a stakeholders | Inspecionado | | | |
...
| GOV-13 | Existem janelas de mudança definidas para sistemas críticos da operação da empresa (ex.: sistema de gestão de risco, ERP)? | Calendário de janelas de mudança ou política formal | Inspecionado | | | |
...
| Ref | Pergunta | Evidência Esperada | Método de Verificação Exigido | Resposta do Entrevistado | Evidência Anexada (Sim/Não/Parcial + referência) | Complemento do Entrevistador |
|---|---|---|---|---|---|---|
| GOV-22 | Existe mapeamento de fornecedores críticos de TI (incluindo fornecedores de sistemas essenciais à operação da empresa, como o sistema de gestão de risco) com classificação de criticidade? | Planilha de fornecedores com criticidade classificada | Inspecionado | | | |
...
| GOV-36 | Softwares de sistemas críticos da operação da empresa (sistema de gestão de risco, ERP) têm licenciamento formalmente vigente e dimensionado para o volume de uso atual? | Contrato/nota fiscal de licenciamento vigente com quantidade contratada | Inspecionado | | | |
