...
cliente: "JC Gestão de Risco"
voltarHref: "/assessment/jc-gestao-de-risco/"
---

# Questionário — Infraestrutura e Redes
Cliente: JC Gestão de Risco | Módulo: Infraestrutura e Redes
Interlocutor: (a definir no kickoff)
...
| Ref | Pergunta | Evidência Esperada | Método de Verificação Exigido | Resposta do Entrevistado | Evidência Anexada (Sim/Não/Parcial + referência) | Complemento do Entrevistador |
|---|---|---|---|---|---|---|
| INF-10 | A rede é segmentada em zonas distintas (administração, servidores, operação da empresa, visitantes/Wi-Fi público)? | Diagrama de VLANs ou configuração de switches/firewall | Inspecionado | | | |
...
| INF-12 | Redes de operação (ex.: sistemas de negócio e bases de dados críticas) estão isoladas da rede corporativa administrativa? | Diagrama de segmentação operação vs. administração + regras de isolamento | Inspecionado | | | |
...
| INF-30 | Existe segregação de dados críticos (ex.: bases de dados de sistemas críticos de negócio) em volumes com política de proteção reforçada (RAID mais robusto, snapshot mais frequente, tier de disco dedicado)? | Mapeamento de volumes por criticidade de dado, com a política de proteção aplicada a cada um | Inspecionado | | | |
...
| INF-39 | Existe plano de expansão de capacidade formalizado para os próximos 6-12 meses, considerando o crescimento projetado da operação da empresa? | Plano de capacidade/expansão documentado, com prazos e investimentos previstos | Inspecionado | | | |
...
## Contratado x Provisionado x Utilizado (Auditoria de Contratos de Infraestrutura)

| Ref | Pergunta | Evidência Esperada | Método de Verificação Exigido | Resposta do Entrevistado | Evidência Anexada (Sim/Não/Parcial + referência) | Complemento do Entrevistador |
|---|---|---|---|---|---|---|
| INF-41 | Qual o volume contratado junto a cada fornecedor de infraestrutura (banda de internet/link entre sites, licenças de virtualização, capacidade de storage) comparado ao volume efetivamente provisionado/instalado hoje? | Contrato vigente por fornecedor/serviço + relatório técnico com o volume provisionado | Inspecionado | | | |
| INF-42 | Desse volume provisionado, qual o percentual efetivamente utilizado (banda consumida, capacidade de storage ocupada, licenças de virtualização ativas) nos últimos 3 meses? | Relatório de utilização real com população total de itens contratados, amostra verificada conforme a Regra de Amostragem e critério de seleção | Testado por amostragem | | | |
| INF-43 | Existe processo formal de revisão periódica de contratos de infraestrutura para identificar sobre-contratação (pagando por capacidade não utilizada) ou sub-contratação (risco de saturação)? | Processo documentado + registro da última revisão de dimensionamento contratual, com ação tomada (renegociação, upgrade, downgrade) | Inspecionado | | | |
